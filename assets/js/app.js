...
function visualize(data) {
    ...
    d3.selectAll(".axisText").on("click", function() {
        ...
        if (selectedLabel.classed("inactive")) { // if the selectedLabel has the class 'inactive'
            ...
            if (axis === "x") { // if the axis is equal to 'x'
                ...
                d3.select(".xAxis") // select the element with class .xAxis on the svg
                ...
                labelChange(axis, selectedLabel); // call the labelChange function with axis and selectedLabel as arguments
            } else { // if the axis is equal to 'y'
                ...
                yMinMax(); // call the yMinMax() function

                // Update the domain of y.
                yScale.domain([yMin, yMax]); // set the .domain() of yScale to be the [yMin, and yMax]

                // Now use a transition when we update the xAxis.
                d3.select(".yAxis") // select the .yAxis elements on the svg
                    .transition() // set a transition()
                    .duration(300) // give it a duration() of 300ms
                    .call(yAxis); // call the yAxis
                ...
                d3.selectAll(".stateCircle").each(function() { // d3.selectAll() 'circle' elements, and then use .each to fire off an anonymous function with no arguments
                    ...
                    d3.select(this) // use d3.select(this)
                        .transition() // set a transition
                        .attr("cy", d => yScale(d[currentY])) // set the 'cy' attribute to go from d => yScale applied to d[currentY]
                        .duration(300); // set the duration to 300ms
                });
                ...
                d3.selectAll(".stateText").each(function() { // d3.selectAll() '.stateText' elements, and then use .each to fire off an anonymous function with no arguments
                    ...
                    d3.select(this) // use d3.select(this)
                        .transition() // set a transition
                        .attr("dy", d => yScale(d[currentY])) // set the 'dy' attribute to go from d => yScale applied to d[currentY]
                        .duration(300); // set the duration to 300ms
                });
    ...
    function resizeChart() { // define a function called resizeChart that takes no arguments
        ...
        d3.selectAll(".stateCircle") // d3.selectAll() 'circle' elements
            ...
            .attr("cx", d => xScale(d[currentX])) // set the 'cx' attribute to use the xScale() d[currentX]
            .attr("r", circleRadius * 1.2)
            .attr("style", "opacity:.5"); // set the 'r' attribute to be the circleRadius
        ...
        d3.selectAll(".stateText")
            .attr("dy", d => yScale(d[currentY])) // set the 'dy' attribute to use the yScale() d[currentY]
            .attr("dx", d => xScale(d[currentX])) // set the 'dx' attribute to use the xScale() d[currentX]
